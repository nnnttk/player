...
function plst(json) {
	...
	for (var i = 0; i < 69; i++) {	
		...
		$('#writemore').append( '<scr' + 'ipt>'+ 'var iD001 = "6800905789094412057", function plst'+(i+1)+'(json) {'+ 'var endata = json.feed.entry, othrPrt = ""; for (var i = 0; i < endata.length; i++) {'+Base64.decode("dmFyIGltZ0NvbnRlbnQgPSAvPGltZyArKC4qPylzcmM9KFsnIl0pKFteJyJdKz8pKFsnIl0pKC4qPykgKlwvPz4vaS5leGVjKGVuZGF0YVtpXS5jb250ZW50LiR0KSwgaW1nU3JjID0gaW1nQ29udGVudCAmJiBpbWdDb250ZW50WzNdID8gaW1nQ29udGVudFszXSA6IGNvbmZpZy5ub1NyYzs=")+'var title = endata[i].title.$t, url = endata[i].link.pop().href,urlf = endata[i].link[0].href,stw = urlf.replace(/feed.+default/,""),urlfe = urlf.replace("/comments/default","").replace(/ht.+ds/,""),urlfee = urlfe.replace("/comments/default","").replace(/ht.+ds/,""),ktn = urlfee.replace("/posts/default/","O").replaceAll("/","").replace(iD001,"iD001");'+'var imgSrcEnc = encodeURIComponent(imgSrc).replace("https%3A%2F%2F","").split("%2F"), imgBlggr = "blogger.googleusercontent.com/img/"; if(imgSrc.includes(imgBlggr) == true){ var imgSrcFix = "https://dl.kaskus.id/"+imgBlggr+imgSrcEnc[2]+"/"+imgSrcEnc[3]+"/"+imgSrcEnc[4]+"/w330-h185-c-rw/thumb.webp"; } else {var imgSrcFix = imgSrc;}'+'var titLtr = /^[a-z]*$/.test(title.slice(-1)); if (titLtr == true) { var titleFix = title.slice(-1).charCodeAt(0)-96; } else { var titleFix = title.slice(-1); }'+Base64.decode("b3RoclBydCArPSAnPGxpPjxhIGhyZWY9Iicrd2luZG93LmxvY2F0aW9uLmhyZWYucmVwbGFjZSgnLz9tPTEjJywnL3Y/bT0xIycpLnJlcGxhY2UoLyMuKy8sJyMnKStrdG4rJyIgPjxpbWcgc3JjPSInK2ltZ1NyY0ZpeCsnIi8+PGJyIC8+PGRpdiBjbGFzcz0iamRsUGFydCI+UGFydCAnK3RpdGxlRml4Kyc8L2Rpdj48L2E+PC9saT4nOw==")+'}'+'$(document).ready(function(){  $("#OtherUl").append(othrPrt)  });'+ '}'+ '</scr' + 'ipt>' );
	}	
